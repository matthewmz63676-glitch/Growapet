...
import me.growapet.plot.PlotVisitManager;
...
public final class GrowAPet extends JavaPlugin {
    ...
    private PlotManager plotManager; private PlotVisitManager plotVisitManager; private PetManager petManager; private EggManager eggManager;
    ...
    private void initializeManagers(){
        ...
        questManager=new QuestManager(this);tradeManager=new TradeManager(this);actionBarManager=new ActionBarManager(this);dailyManager=new DailyManager(this);plotBoostManager=new PlotBoostManager(this);leaderboardManager=new LeaderboardManager(this);tutorialManager=new TutorialManager(this);relicManager=new RelicManager(this);chatGameManager=new ChatGameManager(this);
        CompletableFuture.allOf(plotManager.loadAll(),petManager.loadAll(),eggIncubationManager.loadAll(),eventManager.loadAll(),bossManager.loadAll(),tradeManager.initialize(),plotBoostManager.loadAll(),mobSpawnPointManager.loadAll(),seasonService.load())
    ...
    private void finishEnable(){
        me.growapet.integration.LuckPermsHook.initialize();wallManager.loadAll();registerListeners();virtualTextDisplays.start();mobManager.start();mobSpawnPointManager.start();petManager.start();bossManager.start();eventManager.start();actionBarManager.start();plotBoostManager.start();leaderboardManager.start();relicManager.start();chatGameManager.start();discordIntegration.loadLinks().exceptionally(error->{getLogger().warning("Discord link cache could not load: "+error.getMessage());return null;});discordIntegration.start();
    ...
    private void registerListeners(){for(Listener listener:List.of(new CommandLockdownListener(this),new PlayerListener(this),new ZoneVisibilityListener(this),new ZoneEntityCullingListener(this),new MobDamageListener(this),new EggListener(this),eggIncubationManager,new BossListener(this),plotVisitManager,new SpawnEggListener(this),new BoostItemListener(this),new ChatListener(this),new WallListener(this),new MenuListener(this),new PetListener(this),actionBarManager,relicManager))getServer().getPluginManager().registerEvents(listener,this);}
    private void registerCommands(){command("growapet",new GrowAPetCommand(this));command("plot",new PlotCommand(this));command("pets",new PetsCommand(this));command("stats",new StatsCommand(this));command("boss",new BossCommand(this));command("warp",new WarpCommand(this));command("zones",new ZonesCommand(this));command("visit",new VisitCommand(this));command("leaderboard",new LeaderboardCommand(this));command("getegg",new GetEggCommand(this));command("shop",new ShopCommand(this));command("spawn",new SpawnCommand(this));command("setspawn",new SetSpawnCommand(this));command("store",new StoreCommand(this));command("unlockzone",new UnlockZoneCommand(this));command("quests",new QuestsCommand(this));command("trade",new TradeCommand(this));command("options",new OptionsCommand(this));command("getmob",new GetMobCommand(this));command("mobspawn",new MobSpawnCommand(this));command("getpet",new GetPetCommand(this));command("autokill",new AutoKillCommand(this));command("daily",new DailyCommand(this));command("tutorial",new TutorialCommand(this));command("link",new DiscordCommand(this));command("checklink",new DiscordCommand(this));command("unlink",new DiscordCommand(this));command("season",new SeasonCommand(this));command("cosmetics",new CosmeticsCommand(this));GrowAPetTabCompleter completer=new GrowAPetTabCompleter(this);for(String name:List.of("growapet","plot","pets","stats","boss","warp","zones","visit","leaderboard","getegg","shop","spawn","setspawn","store","unlockzone","quests","trade","options","getmob","mobspawn","getpet","autokill","daily","tutorial","link","checklink","unlink","season","cosmetics")){PluginCommand registered=getCommand(name);if(registered!=null)registered.setTabCompleter(safeCompleter(name,completer));}}
