...
public final class OptionsCommand implements CommandExecutor {
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[]args){
        ...
        if(args.length==1&&(args[0].equalsIgnoreCase("actionbar")||args[0].equalsIgnoreCase("trade")||args[0].equalsIgnoreCase("trades"))){
            boolean actionbar=args[0].equalsIgnoreCase("actionbar");
            String key=actionbar?"actionbar":"trade_requests";
            String display=actionbar?"Action bar":"Trade requests";
            ...
            plugin.getOptionsManager().set(player.getUniqueId(),key,String.valueOf(next)).whenComplete((ignored,error)->
                    Bukkit.getScheduler().runTask(plugin,()->{
                        ...
                        if(error!=null){if(!actionbar)plugin.getTradeManager().onTradePreferenceChanged(player.getUniqueId(),!next);Messages.send(player,"<red>Could not save that preference. Your previous setting was restored.</red>");return;}
