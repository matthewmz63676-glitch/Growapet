...
public final class LeaderboardManager {
    ...
    public void stop(){if(task!=null)task.cancel();task=null;for(Board board:boards.values())if(board.handle!=null)plugin.getVirtualTextDisplays().remove(board.handle);for(VirtualTextDisplayService.Handle handle:personalBoards.values())plugin.getVirtualTextDisplays().remove(handle);personalBoards.clear();boards.clear();}
